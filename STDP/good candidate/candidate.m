% Define delta_t values and expanded parameter pairs with k11 for all combinations
parameter_pairs = [
%candidate 1: suggested in the email, potentially in LTD preferred region
...
%candidate 3: in LTD preferred region, but with LTD blip
    0.9,1.5,0.96,100, 0.25, 0.055,60,1,1.3,3,200,100;   
...
delta_t_values = [-500, -400, -300, -200, -150, -100, -75, -50, -25, -20,-15,-10, 0, 10,20, 25, 50, 75, 100, 200, 300, 400, 500];
...
parfor iter = 1:size(parameter_pairs, 1)
    ...
    k11 = 0.2; % k11 is now part of the parameter_pairs
    hertz=parameter_pairs(iter, 8),
    frequency_time=1000/hertz;
    
    % Define constant parameters
    K0 = 0.5; k1 = 2/10; k2 = 15/10; k3 = 1/10; k4 = a * parameter_pairs(iter,11) / 10; 
    k12 = 15/10; k13 = 1/10; 
    k14 = a * parameter_pairs(iter,12)  / 10; 
    Km1 = 10; Km2 = 0.3; 
    Km12 = b; 
    ...
    timespan =200000; tau_1_ep = 50; tau_2_ep = 5; s = 1; span = [0 timespan]; 
    v = 0.5; init_val = [0.0196, 0.0691, 4.4656, 0];
    ...
    for idx = 1:length(delta_t_values)
        ...
        t_glut_list =500+ t_glut + frequency_time * (0:pairs);
        t_bpap_list = 500+t_bpap + frequency_time * (0:pairs);
    ...
    % Update the title with k11 values
    ...
    filename = sprintf('1:%2f_short_G_VDCC=%.3f_G_NMDA=%.3f_Km12=%.3f_a=%.2f_ratio=%d_k11=%.4f_pre=%.2f_hertz=%.2f_Km=%.1f_pairs=%.1f_k4=%.1f_k14=%.1f.png', c, G_VDCC * 50, G_NMDA * 50, Km12, a, int32(ratio), k11_new,pre,hertz,Km,pairs,parameter_pairs(iter,11)/10,parameter_pairs(iter,12)/10);
    print(filename, '-dpng');
end
